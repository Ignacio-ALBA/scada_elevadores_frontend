// frontend/src/components/pages/InterfazGrafica/vistas/VistaMejorada/MatrizMejorada.jsx
...
const MatrizMejorada = ({
  ...
  const margenSuperior = mostrarEtiquetas ? 40 : 20;
  const margenInferior = 10;

  // ✅ Ancho/alto disponible REAL (basado en el contenedor)
    ...
    >
      ...
      {onMatrizHeightChange && (
        <div className="absolute top-2 right-2 z-20 flex items-center gap-1">
          <div className={`flex items-center gap-0.5 rounded-lg ${isDark ? 'bg-slate-800/80' : 'bg-white/80'} border ${isDark ? 'border-slate-700' : 'border-gray-200'} backdrop-blur-sm p-1 shadow-lg`}>
            <button
              onClick={() => onMatrizHeightChange('increase')}
              className={`p-1.5 rounded-md transition-all duration-200 ${isDark ? 'text-cyan-400 hover:bg-slate-700/50' : 'text-primary-600 hover:bg-gray-200/50'}`}
      ...
      >
        ...
        >
          ...
          {pisos.map((piso, i) => {
            ...
            let pisoColor;
            if (esFilaAdicional) {
              pisoColor = isDark ? '#64748b' : '#94a3b8';
            ...
            const esActivo = esFilaAdicional ? false : activo;
              ...
              <div key={`piso-${piso}`} className="absolute" style={{ top: y, left: 0, right: 0, height: altoCelda }}>
                ...
                <div 
                  ...
                  style={{
                    ...
                    opacity: (esActivo || tieneLlamada) ? 1 : 0.3
                  }}
                />
