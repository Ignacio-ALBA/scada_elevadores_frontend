...
const MatrizPisos = ({
  ...
  const bgColor = isDark ? 'bg-slate-900' : 'bg-gray-100';
  ...
  const leyendaBg = isDark ? 'bg-slate-800/90 border-slate-700' : 'bg-white/90 border-gray-300';
  const leyendaText = isDark ? 'text-slate-300' : 'text-gray-600';
  ...
  return (
    <div ref={containerRef} className={`w-full h-full ${bgColor} relative overflow-auto`}>
      <div 
        className="relative"
        style={{
      ...
      >
            ...
            <div key={piso} className="absolute" style={{ top: y, left: 0, right: 0, height: altoCelda }}>
              <div className="absolute" style={{ left: 8, width: 36, top: '50%', transform: 'translateY(-50%)' }}>
                <IconPiso 
                  numero={piso} 
                  activo={activo}
                  color={activo ? '#22d3ee' : (isDark ? '#334155' : '#9ca3af')}
                  textColor={isDark ? '#ffffff' : '#1e293b'}
                  isDark={isDark}  // ✅ Pasar isDark
                />
              </div>
              <div 
                ...
                style={{
                  ...
                  right: 0,
                  top: '50%',
                  background: activo ? pisoLineaActive : pisoLineaInactive
                }}
              />
            </div>
          );
        ...
        {elevadoresMostrar.map((elevador, j) => {
          ...
          const rielActive = isDark
            ? 'linear-gradient(180deg, rgba(34,211,238,0.3) 0%, rgba(34,211,238,0.1) 50%, rgba(34,211,238,0.3) 100%)'
            : 'linear-gradient(180deg, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0.05) 50%, rgba(0,0,0,0.2) 100%)';
          const rielInactive = isDark
            ...
            <div key={`riel-${elevador.id}`} className="absolute" style={{ left: x - 1, top: offsetY, width: 2, height: altoCelda * numPisos }}>
              <div 
                ...
                style={{
                  ...
                  top: 0,
                  height: '100%',
                  background: activo ? rielActive : rielInactive
                }}
              />
              <div 
                className="absolute w-px"
          ...
          return Array.from({ length: numPuertasFijas }).map((_, idx) => {
            ...
            return (
              ...
              >
                ...
                />
                <div className={`absolute inset-0 flex items-center justify-center text-[8px] font-mono ${isDark ? 'text-slate-500' : 'text-gray-400'}`}>
                  {idx + 1}
                </div>
              </div>
            );
        ...
        {elevadoresMostrar.map((elevador, j) => {
          ...
          return (
            ...
            >
              <span className={`text-[10px] font-mono ${isDark ? 'text-cyan-400/80' : 'text-primary-600/80'} whitespace-nowrap`}>
      ...
      {/* Leyenda */}
      <div className={`absolute bottom-4 right-4 ${leyendaBg} backdrop-blur-sm rounded-lg p-3 border ${borderColor} text-xs flex flex-wrap gap-3 ${leyendaText}`}>
