...
const CarruselElevadores = ({
  ...
  const containerBg = isDark ? 'bg-slate-800/30 border-slate-700' : 'bg-gray-200/50 border-gray-300';
  const textColor = isDark ? 'text-slate-400' : 'text-gray-500';
  const textColorSelected = isDark ? 'text-cyan-400' : 'text-primary-600';
  const buttonColor = isDark ? 'text-slate-400 hover:bg-slate-700 hover:text-white' : 'text-gray-500 hover:bg-gray-200 hover:text-gray-800';
  ...
  if (elevadores.length === 0) {
    return (
      <div className={`${containerBg} px-4 py-6 text-center ${textColor}`}>
  ...
  return (
    <div ref={containerRef} className={`${containerBg} px-3 py-3 w-full border-b`}>
